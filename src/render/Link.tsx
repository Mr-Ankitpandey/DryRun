...
export function Link({ p, scene }: { p: LinkPrim; scene: Scene }) {
  ...
  if (!a && !b) return null;
  const common = { 'data-id': p.id, 'data-style': p.style, initial: inst ? false : { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 }, transition: fade } as const;
  ...
  if (p.style === 'dep') {
    ...
    const by = b.y + b.h / 2;
    return (
      <motion.g {...common} pointerEvents="none">
